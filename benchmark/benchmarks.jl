...
let r = @rule(~x => ~x), rs = RuleSet([r]),
    ...
    fold = if hasmethod(SymbolicUtils.Substituter{true}, Tuple{Dict, Function})
        Val{false}()
    else
        false
    end
    overhead["substitute"]["a"] = @benchmarkable substitute(subs_expr, $(Dict(a=>1)); fold) setup=begin
